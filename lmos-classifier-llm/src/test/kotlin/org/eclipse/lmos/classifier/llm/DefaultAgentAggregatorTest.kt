...
internal class DefaultAgentAggregatorTest {
    @Test
    fun `aggregate returns agents from all providers`() {
        // given
        val someProvider =
            object : AgentProvider {
                override fun provide(request: ClassificationRequest) = listOf(agent("some-agent-1"), agent("some-agent-2"))
            }
        val anotherProvider =
            object : AgentProvider {
                override fun provide(request: ClassificationRequest) = listOf(agent("another-agent-1"))
            }
    ...
    @Test
    fun `aggregate removes duplicate agents by id`() {
        // given
        val someProvider =
            object : AgentProvider {
                override fun provide(request: ClassificationRequest) = listOf(agent("duplicate-agent-1"), agent("some-agent-1"))
            }
        val anotherProvider =
            object : AgentProvider {
                override fun provide(request: ClassificationRequest) = listOf(agent("duplicate-agent-1"), agent("another-agent-1"))
            }
        val aggregator = DefaultAgentAggregator(listOf(someProvider, anotherProvider))
    ...
    @Test
    fun `aggregate ignores provider that throws exception and still collect others`() {
        // given
        val someProvider =
            object : AgentProvider {
                override fun provide(request: ClassificationRequest) = listOf(agent("some-agent-1"))
            }
        val failingProvider =
            object : AgentProvider {
                override fun provide(request: ClassificationRequest): List<Agent> = throw RuntimeException("boom")
            }
